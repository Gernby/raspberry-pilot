git clone https://github.com/gernby/raspberry-pilot raspilot
sudo apt update
sudo apt install -y can-utils network-manager net-tools python3-full python3-pip libusb-1.0-0 libusb-1.0-0-dev influxdb influxdb-client vim htop

python3 -m venv ~/raspilot
cd raspilot
bin/python3 -m pip install -U pip
bin/python3 -m pip install setproctitle pandacan psutil influxdb python-can
...
echo " net.ifnames=0 dwc_otg.lpm_enable=0 dwc_otg.otg_cap=1 modules-load=dwc2" | tee -a /boot/firmware/cmdline.txt
...
rm /home/raspilot/raspilot/panda -r
chown raspilot -R /home/raspilot/raspilot
